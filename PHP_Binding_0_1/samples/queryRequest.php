<?php
  include '../wa_wrapper/WolframAlphaEngine.php';
	...
	$_REQUEST['q'] = '(dy/dx) = (('.$_GET['x1'].'*x) + ('.$_GET['y1'].'*y)) / (('.$_GET['x2'].'*x) + ('.$_GET['y2'].'*y))';
...
<?php
    foreach ( $response->getPods() as $pod ) {
		if($pod->attributes['title'] == 'Input interpretation' || $pod->attributes['title'] == 'Sample solution family' || $pod->attributes['title'] == 'Differential equation solution') {
?>
      <tr>
        <td>
          <h3>
		  	<?php 
		  		...
		  		echo 'Notacion de las primeras derivadas'; 
				if($pod->attributes['title'] == 'Sample solution family')
		  		echo 'Graficas de fase'; 
				if($pod->attributes['title'] == 'Differential equation solution')
		  		echo 'Famila de soluciones'; 
